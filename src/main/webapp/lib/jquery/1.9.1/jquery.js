...
jQuery.each({
	...
	jQuery.fn[ name ] = function( selector ) {
		...
		for ( ; i <= last; i++ ) {
			...
			// Modern browsers can apply jQuery Collectionss as arrays, but oldIE needs a .get()
...
function createTweens( animation, props ) {
	jQuery.each( props, function( prop, value ) {
		var Collections = ( tweeners[ prop ] || [] ).concat( tweeners[ "*" ] ),
			index = 0,
			length = Collections.length;
		for ( ; index < length; index++ ) {
			if ( Collections[ index ].call( animation, prop, value ) ) {
