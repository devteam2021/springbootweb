...
(function() {
	$(document).ready(function() {
		$("select[group-k]").each(function(index, element) {
			var self = $(this);
			var df_val = self.attr("default-value") != undefined ? self.attr("default-value") : "";

			if (self.attr("frist-null") != undefined) {
				self.append("<option value='-1'>--请选择职称--</option>");
			}
			;
			$.ajax({
				type : 'POST',
				dateType : 'JSON',
				url : ctx+'beauticianLevelController/listById',
				success : function(json) {
					if (json.code == "00" && json.resultData.length > 0) {
						...
						for (var i = 0; i < json.resultData.length; i++) {
							...
							self.append("<option value='" + json.resultData[i].id + "' " + selected + ">" + json.resultData[i].levelName + "</option>");
						}
					}
